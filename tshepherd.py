...
def parse_endpoint(target):
    ...
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]{0,63}", session) or not re.fullmatch(r"w" + handle + r":p" + handle, pane):
        raise ValueError("ungültige Endpunktidentität")
    return session, pane
...
class PrimaryRow:
    key: tuple = (PRIMARY,)
    live: str = "unknown"
    reason: str = "Firstmate-Identität noch nicht bestätigt"
    observed: float = 0
    provider: str = ""
    session: str = ""
    pane: str = ""
...
def overview_rows(view, now, ttl):
    ...
    if view.error or not 0 <= now - primary.observed <= ttl:
        primary = replace(primary, live="unknown", reason="Firstmate nicht verfügbar: Messung fehlt/veraltet")
...
def rows_for(snapshot, natives, now, ttl, unavailable=False):
    ...
    for task in snapshot.get("tasks", []):
        ...
        project = clean(backlog.get("repo")) or clean(task.get("project")) or "Projekt unbekannt"
        ...
        if stale or not fresh(current, now, ttl):
            outcome, reason = "unknown", "veraltet / Quelle nicht erreichbar"
        ...
        if stale or not 0 <= now - native.observed <= ttl or native.binding != identity(task):
            live = "unknown"
            reason = reason or "Native-Messung fehlt/veraltet"
        ...
        if not activity:
            ...
            if isinstance(log, dict) and log.get("kind") == "event_history":
                ...
                if isinstance(event, dict):
                    ...
                    if activity:
                        activity = "Historie: " + activity
        rows.append(Row(task, project, title, live, outcome, activity or "keine Aktivität geliefert",
            ...
            try:
                while not completed.is_set():
                    ...
                    if time.monotonic() >= deadline:
                        raise TimeoutError("Zeitlimit: " + Path(argv[0]).name)
                    if os.fstat(out.fileno()).st_size > 8 * 1024 * 1024 or os.fstat(err.fileno()).st_size > 65536:
                        raise ValueError("Antwort überschreitet Größenlimit")
                ...
                err.seek(0)
                if proc.returncode:
                    raise RuntimeError(clean(err.read(1024).decode("utf-8", "replace")) or "Befehl fehlgeschlagen")
                data = json.load(out)
...
class Source:
    ...
    def argv(self, session, *args):
        ...
        if c.lab_session:
            if session != c.lab_session:
                raise ValueError("Lab verweigert fremde Session")
        ...
        try:
            ...
            def call(*args):
                ...
                if self.runner.stop.is_set():
                    raise RuntimeError("abgebrochen")
                if remaining <= 0:
                    raise TimeoutError("Messbudget verbraucht")
            ...
            if not session_confirmed(status, session):
                raise ValueError("Session/Protokoll nicht bestätigt")
            info = next(replies).get("result", {})
            found = info.get("pane", {})
            if info.get("type") != "pane_info" or found.get("pane_id") != pane:
                raise ValueError("Pane-Identität nicht bestätigt")
            ...
            if (agent_info.get("type") != "agent_info" or not expected
                ...
                raise ValueError("Provider/Endpunkt nicht bestätigt")
            ...
            if (process_result.get("type") != "pane_process_info" or process.get("pane_id") != pane
                ...
                raise ValueError("Prozessbeleg fehlt / Shell-only (Registrierung eventuell veraltet)")
            ...
            detail = ("Herdr native: done · beendet, keine Live-Aussage" if raw == "done"
                      else "Native Aktivität unbekannt · Herdr-Registrierung prüfen" if state == "unknown"
                      else "Herdr native: " + clean(raw))
            ...
            reason = clean(str(error)) if isinstance(error, ValueError) else "Quelle nicht lesbar · Firstmate-/Herdr-Zugriff prüfen"
    ...
    def primary(self, deadline):
        ...
        try:
            def run(argv):
                remaining = deadline - time.monotonic()
                if remaining <= 0:
                    raise TimeoutError("Firstmate-Messbudget verbraucht")
            ...
            if owner.get("home") != self.config.home:
                raise ValueError("Owner gehört anderem Home")
            ...
            if "HERDR_SESSION" not in env:
                ...
                if socket != str(Path.home() / ".config/herdr/herdr.sock"):
                    raise ValueError("Owner-Session nicht bestätigt")
                session = "default"
            if env.get("HERDR_ENV") != "1":
                raise ValueError("Owner läuft nicht bestätigt in Herdr")
            ...
            if not session_confirmed(status, session) or status["server"]["socket"] != socket:
                raise ValueError("Owner-Session/Socket nicht bestätigt")
            ...
            if (info.get("type") != "pane_info" or agent_info.get("type") != "agent_info"
                ...
                raise ValueError("Owner-Endpunkt/Registrierung nicht bestätigt")
            ...
            if (process.get("type") != "pane_process_info" or details.get("pane_id") != pane
                ...
                raise ValueError("Native Owner-Shell nicht bestätigt")
            ...
            if run(reader + [str(shell)]) != owner:
                raise ValueError("Owner/Abstammung während Prüfung geändert oder unbestätigt")
            ...
            if (check.get("type") != "pane_info" or exact.get("pane_id") != pane
                ...
                raise ValueError("Physischer Owner-Endpunkt während Prüfung geändert")
            ...
            reason = ("Herdr native: done · beendet, keine Live-Aussage" if agent.get("agent_status") == "done"
                      else "Native Aktivität unbekannt · Herdr-Registrierung prüfen") if state == "unknown" else "Primärer Chat · Enter wechselt"
            ...
            reason = clean(str(error)) if isinstance(error, ValueError) else "Quelle nicht lesbar · Owner-/Herdr-Zugriff prüfen"
            return PrimaryRow(reason="Firstmate nicht verfügbar: " + reason, observed=time.time())

    def primary_target(self, selected):
        current = self.primary(time.monotonic() + 12)
        if not current.physical or current.key != selected:
            raise ValueError("Fokus verweigert: Firstmate entfernt/ersetzt/unbestätigt · " + current.reason)
    ...
    def focus_snapshot(self, selected):
        ...
        if task is None:
            raise ValueError("Fokus verweigert: Worker entfernt/ersetzt")
        try:
            self.check_owner(task)
        except OSError as error:
            raise ValueError("Fokus verweigert: Metadaten nicht lesbar / entfernt") from error
        return snapshot
    ...
    def check_owner(self, task):
        ...
        if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*", task_id):
            raise ValueError("Fokus verweigert: ungültige Metadatenidentität")
        ...
        if (not isinstance(metadata, dict) or metadata.get("present") is not True
            ...
            raise ValueError("Fokus verweigert: Metadaten nicht an dieses Home gebunden")
        fd = os.open(expected, os.O_RDONLY | os.O_NONBLOCK | os.O_NOFOLLOW)
        try:
            before = os.fstat(fd)
            if not stat.S_ISREG(before.st_mode) or before.st_size > 65536:
                raise ValueError("Fokus verweigert: Metadaten nicht begrenzt lesbar")
            ...
            if len(data) > 65536 or stamp(before) != stamp(after) or stamp(after) != stamp(current):
                raise ValueError("Fokus verweigert: Metadaten während Prüfung ersetzt")
        finally:
            os.close(fd)
        ...
        if b"\0" in data:
            raise ValueError("Fokus verweigert: binäre Metadaten")
        fields = {}
        ...
        if (fields.get("spawn_gen") != task.get("spawn_gen")
            ...
            raise ValueError("Fokus verweigert: Worker entfernt/ersetzt oder fremd")
    ...
    def focus_target(self, selected):
        ...
        if (latest_time is None or not -2 <= time.time() - latest_time <= self.config.ttl
            ...
            raise ValueError("Fokus verweigert: Auswahl während der Prüfung geändert/veraltet")
        session, pane = endpoint(task)
        ...
        if (check.get("type") != "pane_info" or exact.get("pane_id") != pane
            ...
            raise ValueError("Fokus verweigert: physischer Endpunkt geändert")
    ...
    def focus(self, selected, physical=()):
        ...
        if physical and native.physical != physical:
            raise ValueError("Fokus verweigert: ausgewählter physischer Endpunkt ersetzt")
        ...
        if (result.get("type") != "agent_info" or focused.get("pane_id") != pane
            ...
            raise ValueError("Fokusanfrage gesendet, Bestätigung unklar")
        try:
            ...
            if current.physical != native.physical:
                raise ValueError("physischer Endpunkt geändert")
            ...
            if (projection.get("type") != "tab_info" or tab.get("focused") is not True
                ...
                raise ValueError("Tab-Fokusantwort unklar")
            ...
            if (confirmation.get("type") != "agent_info" or agent.get("focused") is not True
                ...
                raise ValueError("abschließender Agent-Fokus unklar")
        except (ValueError, OSError, RuntimeError, TimeoutError, AttributeError, TypeError) as error:
            raise ValueError("Agent-Fokus bestätigt; Tab-Wechsel nicht bestätigt: " + clean(str(error))) from error
        return "Agent-/Tab-Fokus serverseitig bestätigt: " + label
        ...
        if kind == "snapshot":
            ...
            if self.selected:
                ...
                if self.selected_physical and physical and physical != self.selected_physical:
                    ...
                    self.message = "Zielidentität geändert · bitte erneut wählen"
...
def render_lines(view, rows, width, height, busy, now):
    ...
    if height < 16 or width < 28:
        return [(fit(text, max(0, width - 1)), spans) for text, spans in [
            styled(("TShepherd — Terminal zu klein", 5)),
            styled(("Mind. 28 Spalten / 16 Zeilen; q beendet", 7))][:height]]
    ...
    age = f"{max(0, now - view.last_success):.0f}s" if view.last_success else "nie"
    wide = width >= 78
    lines = [blank]
    ...
    for i, label in enumerate(labels):
        ...
        if wide:
            brand = ("TShepherd", "Firstmate fleet", "", "Live · lokal", "", "")[i]
            prefix = "  " + column(brand, 22)
        else:
            prefix = "  "
        segments = [(prefix, 6 if i == 0 else 7), (f" {value:>3} ", 11 + i), (" " + column(label, 10), 7)]
        if wide and i == 0:
            segments += [(f"   letzter Erfolg {age}", 7)]
        if wide and i == 1:
            segments += [("   " + ("aktualisiert …" if busy else "automatische Aktualisierung"), 7)]
        if wide and i == 2:
            segments += [("   nur dieses Home; keine Unterhomes", 7)]
        if wide and i == 4:
            segments += [("   Aufgabe done; überlappt mit Live", 7)]
        if wide and i == 5:
            segments += [("   Messung fehlt / nicht bestätigt", 7)]
        lines.append(styled(*segments))
    if not wide:
        lines[0] = styled(("  TShepherd", 6), (f"  ·  Erfolg {age}" + (" · lädt" if busy else ""), 7))
    lines.append(blank)
    if primary is not None:
        mark = ">" if primary.key == view.selected else " "
        status = primary.live if primary.physical else "nicht verfügbar"
    ...
    if wide:
        lines.append(styled(("  #    " + column("Worker", title_width) + "  " + column("Agent", 7)
                             + "  " + column("Live", 7) + "  " + column("Aufgabe", 8)
                             + "  Letzte bekannte Aktivität", 7)))
    ...
    for number, row in enumerate(rows, 1):
        ...
        if wide:
            ...
            body.append(styled(*lead, (column(row.title, title_width), state_color), ("  ", 0),
                               ...
                               (column(row.live, 7), state_color), ("  ", 0),
            ...
            body.append(styled((f"       {row.live} · Aufgabe {row.outcome} · ", state_color), (row.activity, 7)))
        if row.key == view.selected:
            chosen_end = len(body) - 1
    if not rows:
        body = [styled(("  Keine Worker gemessen" if not view.last_success else "  Keine Worker im Snapshot", 7))]
    ...
    if inventory.get("valid") is False:
        warning = "Inventar unvollständig: " + clean(inventory.get("reason")) + (" | " + warning if warning else "")
    ...
    lines.append(styled(("  " + (warning or "Nur eigenes FM_HOME; Secondmate-Kinder nicht rekursiv"), 5 if view.error else 7)))
    lines.append(styled(("  ● working  ! waiting=blocked  ○ idle  ? unknown", 7), ("   completed=Aufgabe done", 4)))
    keys = "  ↑/↓ j/k wählen · Enter Fokus · R neu erkennen · q / Ctrl+C" if wide else "  j/k · Enter · R neu · q"
    ...
    try:
        while True:
            ...
            if key in (curses.KEY_UP, ord("k")):
                ...
                view.message = ("Neuerkennung angefordert · nur lesen" if poller.request_refresh()
                                else "Neuerkennung läuft bereits · nur lesen")
            elif key in (10, 13, curses.KEY_ENTER):
                if not view.selected or view.selected not in [row.key for row in rows] or view.error:
                    view.message = "Keine gültige Auswahl / Quelle nicht erreichbar"
                elif poller.request_focus(view.selected, view.selected_physical):
                    view.message = "Prüfe Auswahl vor Fokus …"
                else:
                    view.message = "Fokusprüfung läuft bereits"
    finally:
        poller.close()
...
def main():
    ...
    parser.add_argument("--fm-home", default=os.environ.get("TSHEPHERD_FM_HOME"))
    parser.add_argument("--firstmate-root", default=os.environ.get("TSHEPHERD_FIRSTMATE_ROOT"))
    parser.add_argument("--herdr", default="herdr", help="Pfad zum kompatiblen Herdr-CLI")
    parser.add_argument("--lab-helper", default="")
    parser.add_argument("--lab-session", default="")
    parser.add_argument("--lab-snapshot", default="", help="nur im isolierten Lab: JSON statt Firstmate-Aufruf")
    args = parser.parse_args()
    if not args.fm_home or not args.firstmate_root:
        parser.error("--fm-home und --firstmate-root explizit angeben (oder TSHEPHERD_* setzen)")
    if bool(args.lab_helper) != bool(args.lab_session) or (args.lab_session and not re.fullmatch(r"fm-lab-[A-Za-z0-9][A-Za-z0-9_-]*", args.lab_session)):
        parser.error("Lab braucht Helper und eine nicht-default fm-lab-* Session")
    if args.lab_snapshot and not args.lab_session:
        parser.error("--lab-snapshot nur mit --lab-helper und --lab-session")
    ...
    try:
        ...
        parser.exit(1, "Terminal nicht verfügbar: " + str(error) + "\n")
